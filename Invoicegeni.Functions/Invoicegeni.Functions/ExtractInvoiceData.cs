        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++)
        {
            try
            {
                ...
                if (name.EndsWith(".pdf", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (documentTitle.ToLower().ToString() == "invoice")
                    {
                    ...
                    {
                        string archiveUri = await BackupProcessor.ArchiveTheProcessedFile(name, "invoice", "InvalidFile", log);
                        await BackupProcessor.InsertInvalidOrDuplicateFile(Environment.GetEnvironmentVariable("SqlConnectionString"), name, "invoice", "InvalidFileType", archiveUri,log);
                    }
                }
                else
                {
                    string archiveUri = await BackupProcessor.ArchiveTheProcessedFile(name, "invoice", "InvalidFile", log);
                    await BackupProcessor.InsertInvalidOrDuplicateFile(Environment.GetEnvironmentVariable("SqlConnectionString"), name, "invoice", "InvalidFileFormat", archiveUri, log);
                }
